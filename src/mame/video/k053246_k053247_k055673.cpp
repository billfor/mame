...
k053246_055673_device::k053246_055673_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, type, tag, owner, clock),
	  ...
	  video_latency::interface(mconfig, *this, 3),
	  m_dmairq_cb(*this),
	  m_dmaact_cb(*this),
...
void k053246_055673_device::set_info(const char *spriteram_tag)
{
...
WRITE16_MEMBER(k053246_055673_device::oms_w)
{
	...
	if(ACCESSING_BITS_0_7 && (data & 0x3f) != (m_oms & 0x3f)) {
		...
		if(diff)
			logerror("oms_w sd0en=%s dma=%s res=%s mode=%d flip=%c%c (%x)\n",
...
void k053246_055673_device::ksnotifier_w(int clk, int hv, int hfp, int hs, int hbp, int vv, int vfp, int vs, int vbp)
{
	m_y_offset = 1 - (vfp + vs + vbp);
	m_x_offset = 1 - (hs + hbp) + video_latency_get();
}
...
void k053246_055673_device::decode_sprite_roms()
{
	...
	gfx_entries[0].total_color_codes = 1;
...
void k053246_055673_device::flow_render_register_renderers()
{
...
void k053246_055673_device::render(const rectangle &cliprect)
{
	...
	int offx = (m_x_offset - m_hscr) & 0x3ff;
	int offy = (m_y_offset - m_vscr) & 0x3ff;
