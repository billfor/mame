...
namespace netlist
{
	...
	NETLIB_OBJECT(log)
	{
		...
		NETLIB_UPDATEI()
		{
			...
			m_strm->writeline(plib::pfmt("{1} {2}").e(netlist().time().as_double(),".9").e(static_cast<double>(INPANALOG(m_I))));
		}
	...
	NETLIB_OBJECT_DERIVED(logD, log)
	{
		...
		NETLIB_UPDATEI()
		{
			m_strm->writeline(plib::pfmt("{1} {2}").e(netlist().time().as_double(),".9").e(static_cast<double>(INPANALOG(m_I) - INPANALOG(m_I2))));
		}
