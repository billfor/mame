...
namespace netlist
{
	...
	NETLIB_UPDATE(7483)
	{
		netlist_sig_t a = (INPLOGIC(m_A1) << 0) | (INPLOGIC(m_A2) << 1) | (INPLOGIC(m_A3) << 2) | (INPLOGIC(m_A4) << 3);
		netlist_sig_t b = (INPLOGIC(m_B1) << 0) | (INPLOGIC(m_B2) << 1) | (INPLOGIC(m_B3) << 2) | (INPLOGIC(m_B4) << 3);

		unsigned r = a + b + INPLOGIC(m_C0);
