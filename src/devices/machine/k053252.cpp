...
============================================================================================================================
     ...
     Names from GX, look similar, all values big-endian, write-only:

    0-1: bits 9-0: HC        - Total horizontal count (-1)  Hres ~ (HC+1) - (HFP-1) - (HBP+1) - 8*(HSW+1)
	2-3: bits 8-0: HFP       - HBlank front porch (+1)
    4-5: bits 8-0: HBP       - HBlank back porch (-1)
    6  : bits 7-0: INT1EN
    7  : bits 7-0: INT2EN
	...
	c  : bits 7-4: VSW       - V-Sync Width (-1)
	c  : bits 3-0: HSW       - H-Sync Width (/8-1)
    d  : bits 7-0: INT-TIME
    e  : bits 7-0: INT1ACK
    f  : bits 7-0: INT2ACK
...
Signal inventory:
    NVSY, NHSY: sync signals, active on 0
    NVBK, NHBK: blank signals, active on 0
    FCNT: frame count?  Synced with vsync start, toggles every two syncs (4-frame cycle)

Beware, all line-level signals (vblank, vsync, irq) are synchronized
with hblank start, not hsync.
...
void k053252_device::update_screen()
{
	...
	int hc = (m_hc & 0x3ff) + 1;
	int hfp = (m_hfp & 0x1ff) - 1;
	int hbp = (m_hbp & 0x1ff) + 1;
	...
	// Line 0 is just after end of vblank
	// Column 0 is just after end of hblank
	// Everything line-based is synchronized with start of hblank

	// Screen vblank happens at coordinates (width, height-1) and ends at (width, vc-1)
	// Vsync happens at (width, height+vfp) and ends at (width, vc-vbp)
	// Hsync happens at (width + hfp, line) and ends at (hc - vfp, line)
	// Hblank happens at (width, line) and ends at (hc - 1, line)
	...
	logerror("screen: %dMHz %d (%d - %d - %d - %d) %d (%d - %d - %d - %d) vbl %gHz\n",
			 clock()/1000000,
			 hc, width,  hfp, hsw, hbp,
			 vc, height, vfp, vsw, vbp,
			 1/m_frame.as_double());

	logerror("MCFG_SCREEN_RAW_PARAMS(%d, %d, %d, %d, %d, %d, %d)\n",
			 clock(),
			 hc, 0, width,
			 vc, 0, height);
	...
	m_ksnotifier_cb(clock(), width, hfp, hsw, hbp, height, vfp, vsw, vbp);
}
...
void k053252_device::frame_transition()
{
	...
	case FT_WAIT_VSYNC_IN:
		...
		m_fcnt++;
		if((m_fcnt & 3) == 0)
			m_fcnt_cb(ASSERT_LINE);
		else if((m_fcnt & 3) == 2)
			m_fcnt_cb(CLEAR_LINE);
		break;
