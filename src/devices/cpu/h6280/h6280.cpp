...
uint32_t h6280_device::opcode_alignment() const
{
	return 1;
}
...
offs_t h6280_device::disassemble(std::ostream &stream, offs_t pc, const data_buffer &opcodes, const data_buffer &params, uint32_t options)
{
	extern CPU_DISASSEMBLE( h6280 );
	return CPU_DISASSEMBLE_NAME(h6280)(this, stream, pc, opcodes, params, options);
}
