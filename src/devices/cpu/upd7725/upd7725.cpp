...
uint32_t necdsp_device::opcode_alignment() const
{
	return 1;
}
...
offs_t necdsp_device::disassemble(std::ostream &stream, offs_t pc, const data_buffer &opcodes, const data_buffer &params, uint32_t options)
{
	extern CPU_DISASSEMBLE( upd7725 );
	return CPU_DISASSEMBLE_NAME(upd7725)(this, stream, pc, opcodes, params, options);
}
